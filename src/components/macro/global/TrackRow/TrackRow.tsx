...
const TrackRow = ({musicUrl, nameTrack, artistId, artistName, album, time}: TrackRowProps): JSX.Element => {
  return (
    <tr className={styles[`trackRow`]}>
      <td><img src={musicUrl}/></td>
      <td>{nameTrack}</td>
      <td><a href={`/artists/${artistName.replace(" ", "")}/${artistId}`}>{artistName}</a></td>
      <td>{album}</td>
      <td>{time}</td>
